...
import { SERVICE_TIER_LABELS, serviceCardDuration, serviceCardFacts, serviceCardHref, serviceCardPrice } from "@/lib/service-cards";
import { siteUrl } from "@/lib/site-url";
import { PAYMENT_METHOD_LABELS, SERVICE_MODE_LABELS } from "@/types";

export async function serviceDetailMetadata(serviceSlug: string, providerSlug?: string): Promise<Metadata> {
...
export async function ServiceDetailPage({ serviceSlug, providerSlug }: { serviceSlug: string; providerSlug: string }) {
  const card = await findPublicServiceCard(serviceSlug);
  ...
  const jsonLd = {
    ...
    name: card.title,
    description: card.description,
    image: card.images.length ? card.images.map((image) => absolute(image.url)) : undefined,
    url: absolute(serviceCardHref(card)),
    provider: { "@type": profile.type === "business" ? "Organization" : "Person", name: profile.name, url: absolute(`/profesionales/${profile.slug}`) },
    ...
    <>
      ...
      <main className="shell py-6 sm:py-10">
        <nav className="mb-5 flex flex-wrap items-center gap-1.5 text-[13px] text-ink-soft" aria-label="Navegación">
          ...
          <Link href={`/profesionales/${profile.slug}`} className="hover:text-brand-800">{profile.name}</Link><Icon name="chevron_right" className="text-[15px]" />
          ...
          <div className="flex min-w-0 flex-col gap-6">
            <section className="overflow-hidden rounded-card border border-line bg-white shadow-panel">
              <div className="relative aspect-[16/8] max-h-[480px] overflow-hidden bg-card-gradient">
                {card.imageUrl ? (
                  ...
                  <img src={card.imageUrl} alt={`Portada de ${card.title}`} className="h-full w-full object-cover" />
                ) : (
            ...
            {samples.length ? (
              <section className="rounded-card border border-line bg-white p-4 shadow-card sm:p-5">
                <h2 className="mb-1 text-[18px] font-bold text-ink">Muestras del servicio</h2>
                <p className="mb-4 text-[13.5px] text-ink-soft">Fotos adicionales de esta propuesta.</p>
