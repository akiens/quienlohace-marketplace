...
- Perfiles y cartas se presentan en una sola lista, identificando el tipo de cada resultado. Primero se determina la elegibilidad por texto, filtros y visibilidad; pagar nunca habilita un resultado que no coincide.
- Entre los resultados elegibles, el plan vigente del proveedor tiene prioridad estricta: **Platino → Oro → Cobre**. Cobre es el nivel gratuito de BR-006, no existe un cuarto nivel gratis. La prioridad se aplica tanto al perfil como a sus cartas y respeta la vigencia de beneficios de BR-008 y los cambios de BR-009.
- Se agotan los resultados elegibles de cada plan antes de pasar al siguiente. Dentro de cada plan se ordena por nivel de evidencia y, dentro de cada nivel, se intercala una entidad por proveedor en cada ronda antes de repetirlo. Para un mismo proveedor y nivel, sus cartas pertinentes preceden a su perfil. La intercalación no adelanta resultados de un plan inferior; si queda un solo proveedor en un grupo, puede aparecer consecutivamente.
- Dentro de cada grupo de plan y evidencia, el orden de proveedores se decide por su mejor relevancia textual, luego por reputación y finalmente por ID estable. Sus cartas se ordenan por relevancia, reputación e ID estable. La paginación continúa este orden global sin reiniciar las rondas, omitir entidades ni repetirlas.
...
- Una vez aplicada la prioridad de plan, la relevancia de la coincidencia se aplica antes que la reputación. Dentro del mismo plan, la oferta explícita —nombre, frase completa o alias validado en el servicio propio— precede a la coincidencia que sólo se apoya en una especialidad. La diversidad opera dentro de cada grupo de plan y evidencia y nunca elimina resultados pertinentes.
